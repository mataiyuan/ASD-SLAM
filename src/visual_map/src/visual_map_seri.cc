...
namespace vm{
    ...
    void save_visual_map(VisualMap& map, std::string file_addr){
	    ...
	    //std::cout<<"desc_width"<<desc_width<<std::endl;
            for(int j=0; j<desc_count; j++){
                for(int k=0; k<desc_width; k++){
	    	   //std::cout<<"desc_width"<<desc_width<<std::endl;
		   //std::cout<<(float)frame_p->descriptors(k, j)<<",";
                    output.write( (const char*)&frame_p->descriptors(k, j), 4 );////
                }
        ...
        for(int i=0; i<frames_size; i++){
            ...
            for(int j=0; j<desc_count; j++){
                for(int k=0; k<desc_width; k++){
                    //char temp_c;
                    //input.read(&temp_c, 1 );
                    frame_p->descriptors(k, j)=getFromFileF(input);
		    ///std::cout<<"desc_width"<<desc_width<<std::endl;
		    //std::cout<<frame_p->descriptors(k, j)<<","<<std::endl;
