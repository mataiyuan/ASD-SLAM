...
const float ORBmatcher::TH_HIGH = 1.5;
...
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th)
{
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)//遍历所有MapPoints
    {
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            const cv::Mat &d = F.mDescriptors.row(idx);////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!这是在利用描述子
...
int ORBmatcher::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
{
    ...
    const DBoW2::FeatureVector &vFeatVecKF = pKF->mFeatVec;    //获取关键帧的单词向量 正向索引

    int nmatches=0;

    vector<int> rotHist[HISTO_LENGTH];///////////??????????????????????????????????????????????????啥意思
    ...
    while(KFit != KFend && Fit != Fend)
    {
        ...
        if(KFit->first == Fit->first)//若两者拥有相同节点(可能含有同一个特征点)
        {
            ...
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                ...
                const cv::Mat &dKF= pKF->mDescriptors.row(realIdxKF);//地图点对应的描述子

                float bestDist1=256;
                int bestIdxF =-1 ;
                float bestDist2=256;

                for(size_t iF=0; iF<vIndicesF.size(); iF++)                //当前帧的同一节点包含的特征点索引
                {
                    ...
                    const float dist =  DescriptorDistance(dKF,dF);//计算两个描述子之间的距离
...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    int nmatches=0;
    vnMatches12 = vector<int>(F1.mvKeysUn.size(),-1);/////这里面装的是F1 里面的 点与F2里面点匹配的那个F2里面点的索引值
    ...
    vector<float> vMatchedDistance(F2.mvKeysUn.size(),100);//////////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!需要改动
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        cv::Mat d1 = F1.mDescriptors.row(i1);//////!!!!!!!!!!!!!!!!!!!!需要改动
...
{    
    const DBoW2::FeatureVector &vFeatVec1 = pKF1->mFeatVec;//关键帧1的特征向量//////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!需要注意
    const DBoW2::FeatureVector &vFeatVec2 = pKF2->mFeatVec;
    ...
    while(f1it!=f1end && f2it!=f2end)
    {
		...
		// 获取pKF1中属于该node节点的所有特征点索引
                ...
                const cv::Mat &d1 = pKF1->mDescriptors.row(idx1);///!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!需要修改
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    for(int i=0; i<nMPs; i++)
    {
        ...
        if(PO.dot(Pn)<0.5*dist3D)//////0.5就决定了为60度  这也就是向量的点积公式
            continue;
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, KeyFrame *pKF, const set<MapPoint*> &sAlreadyFound, const float th , const float ORBdist)/////////这里之前是const int  现在修改为const float
{
    int nmatches = 0;
...
{
	cv::Mat taf ,tbf;
	vector<float> desA_  ,desB_;
	a.convertTo(taf, CV_64F);
	desA_ =  (vector<float>)taf.reshape(1, 1);
	b.convertTo(tbf, CV_64F);
	desB_ =  (vector<float>)tbf.reshape(1, 1);
	float sqd = 0.;
	for(int i = 0; i < 128; i += 4)
	{
	  sqd += (desA_[i  ] - desB_[i  ])*(desA_[i  ] - desB_[i  ]);
	  sqd += (desA_[i+1] - desB_[i+1])*(desA_[i+1] - desB_[i+1]);
	  sqd += (desA_[i+2] - desB_[i+2])*(desA_[i+2] - desB_[i+2]);
	  sqd += (desA_[i+3] - desB_[i+3])*(desA_[i+3] - desB_[i+3]);
	}
